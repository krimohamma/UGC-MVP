...
* **Production incident: signup broken, wrongly attributed to a "Resend" change that never existed in this repo (2026-07-15)**: the user reported urgent signup breakage, describing it as caused by a prior session's "password reset via generateLink + Resend." Checked first, per instruction, before touching anything: `git status`/`git log --all`/`git diff` against `origin/main` showed a completely clean tree and exactly two commits, neither Resend-related; grepped the whole working tree for `resend`/`generateLink`/`RESEND_API_KEY`/`NEXT_PUBLIC_APP_URL` — zero matches; `list_edge_functions` on the live project returned empty. **No such change exists anywhere** — nothing to branch off or preserve. Diagnosed the *real* cause from live Auth logs instead: **Custom SMTP was configured in the Supabase Dashboard (Authentication → Emails → SMTP Settings) pointed at a Gmail account whose credentials Google was rejecting** (`535 5.7.8 ... BadCredentials`) — GoTrue sends the confirmation email synchronously inside `POST /signup`, so every signup 500'd. This is a dashboard setting, invisible to git, which is why "the last session's change" looked like it left no trace — because whatever prior work the user has in mind wasn't in this repository. The user disabled Custom SMTP (falling back to Supabase's built-in mailer), which fixed email delivery but **surfaced a second, genuinely pre-existing bug that had simply never been reached before now** (production had never had a signup complete past the email-send step until this): `lib/auth/actions.ts`'s `signup()` inserted the new user's `public.users` row using the RLS-scoped client, but this project requires email confirmation, so `supabase.auth.signUp()` does not establish a session in that same request — `auth.uid()` is null, and `0002_rls_policies.sql`'s `users_insert_self` policy (`auth.uid() = id`) rejects the insert. That migration's own comment had flagged this exact scenario in advance ("if the project requires email confirmation before a session exists, this policy alone won't cover the signup insert... `lib/auth/actions.ts` will need to use the service-role client for that one insert instead") but the fix was never applied. Result: `auth.users` got a real, even auto-confirmed row, `public.users` got nothing, and the user saw a raw Postgres error — "shows error but creates account," exactly as reported. **Fix**: that one insert now goes through `createAdminClient()` (safe here specifically because `id` is `data.user.id` from Supabase's own trusted `signUp()` response, never client input — documented in both `lib/supabase/admin.ts`'s doc comment and inline). Confirmed via direct SQL against the live project that the reported test account is exactly this: a real, confirmed `auth.users` row with zero matching `public.users` rows — left as-is and flagged to the user rather than deleted (a delete of a live production auth record was correctly blocked by the environment's own safety layer pending explicit user confirmation). **Separately reported by the user in the same message, not yet fixed — still a dashboard step, not code**: the reset-password email link lands on the homepage. Live Auth logs confirm why: `/verify` requests are still coming back with `referer` showing the bare site root, not `/auth/confirm?...` — meaning the two email-template edits from the previous entry (Confirm signup and Reset Password, both needing the `{{ .SiteURL }}/auth/confirm?token_hash=...` link) were never actually applied in the Supabase Dashboard. The code (`app/auth/confirm/route.ts`) has been correct and deployed since the previous entry; nothing more to build here — a human still needs to make those two template edits (exact text under the Auth section above) for the reset link to stop landing on the homepage. `tsc`/`next build` clean after the signup fix; `scripts/verify-lifecycle.ts` unaffected (it doesn't exercise `signup()`'s email-confirmation path — it creates test users directly via the service-role admin API, the same client this fix now also uses for the app's own signup path).
* **Production report of `/auth/confirm` 404 — matcher theory investigated and not confirmed; two independently-justified hardenings applied instead (2026-07-15)**: the user reported the confirm link 404ing and suspected `proxy.ts`'s i18n-middleware matcher was locale-prefixing `/auth/confirm` to a non-existent `/fr/auth/confirm`. **Checked before changing anything**: `proxy.ts`'s matcher (`"/((?!api|trpc|_next|_vercel|auth|.*\\..*).*)"`) already excludes `auth` and has since the route was introduced (see the earlier `/auth/confirm` entry) — confirmed both by compiling the exact pattern in isolation and, more conclusively, by running the local dev server and requesting `/auth/confirm` with the *exact* shapes seen in the real failing link (an absolute-URL `next`, a `pkce_`-prefixed `token_hash`, a fake token): every request correctly reached the route handler and redirected sensibly — no 404, no locale-prefix rewrite, in any case. Also checked and ruled out the other candidates from the task: no conflicting `app/[locale]/auth/*` path, no rewrites/redirects in `next.config.ts`. **Did not change the matcher** — there was nothing to fix there, and changing working, correctly-reasoned config to match an unconfirmed theory would be worse than leaving it. Likely real explanations for what the user saw, none of which are in this app's code: a stale/cached 404 from before the route existed (Vercel/CDN edge caching), or testing against a deployment/domain that predates the `/auth/confirm` commit. Separately, two things from the same report *were* real and got fixed on their own merits, independent of the 404 theory: (a) **open redirect** — `next` (from `{{ .RedirectTo }}`) was used as a raw redirect target with no validation; since email templates embed it as user-visible URL text, a crafted link could set `next` to an attacker's domain and get bounced there right after a legitimate `verifyOtp` success. Added `sanitizeNext()` in `app/auth/confirm/route.ts`: accepts a same-origin absolute URL (reduced to its path) or a bare path starting with `/`, otherwise falls back to the locale homepage — verified against both real production-shaped inputs and synthetic open-redirect attempts (`https://evil.example.com/...`, protocol-relative `//evil.example.com`), all correctly blocked. (b) **`token_hash` arriving `pkce_`-prefixed** — confirmed via `search_docs` that `resetPasswordForEmail` is one of the flows PKCE applies to, and that every documented `verifyOtp` usage treats `token_hash` as an opaque string with no client-side parsing — so the existing code (which already passes it through unchanged) was correct; documented this reasoning inline rather than "fixing" something that wasn't broken. `tsc`/`next build` clean; regression-checked `/`, `/fr`, `/ar`, unauthenticated `/fr/dashboard` and `/fr/admin` redirects, and `scripts/verify-lifecycle.ts` (42/42) all still pass.
* **Two more real, confirmed `/auth/confirm` defects: GET redeeming on prefetch, recovery landing on the dashboard (2026-07-15)**: production evidence this time was unambiguous and directly reproducible by reading the code, not just a report to investigate — GET called `verifyOtp` directly (confirmed by re-reading the route: the GET handler did the redemption itself, no button, no POST step), so any email scanner's prefetch consumed the one-time token and logged the user in without them ever clicking anything; separately, a successful recovery verify fell back to `next ?? `/${locale}/dashboard`` whenever `next` was missing or not `/reset-password`, landing a just-verified user signed-in on the dashboard with their *old* password still active — never showing a way to actually set a new one. (The task described these as regressions from a "prior session whose work was partially lost" — `git log --all` shows no such button/POST design ever existed in this repo; only two auth-related commits predate this one, both GET-only. Whatever that framing refers to isn't recoverable from this repository's history, but the fixes below are correct and address confirmed, currently-reproducible bugs regardless.) **Fix**: split into `GET` (validates `token_hash`/`type` only, renders a small dependency-light HTML page — hand-built in the route handler, not a React page, since the URL must stay locale-less while the button text is locale-aware via `next` — with a form that `POST`s the same params back) and `POST` (the only place `verifyOtp` is called now). A scanner's GET never triggers a `POST`, so it can no longer consume the token. On a successful `POST`, `type=recovery` now unconditionally redirects to `/${locale}/reset-password`, ignoring `next` — recovery must always land on the password form; `type=email`/`signup` keeps the sanitized `next` with a bare-locale-homepage fallback (not the dashboard). Added graceful handling for a legacy `?code=`-shaped link (no `token_hash`) rather than a raw "missing token_hash" error. `logAuthEvent()` now logs `confirmRender` (GET) and `confirmRedeem` (POST) as distinct actions — a scanner prefetch shows up as a `confirmRender` with no matching `confirmRedeem`, which wasn't visible before. `sanitizeNext()` and the `pkce_` token_hash passthrough from the previous entry are unchanged. New `auth.confirm.*` keys in both `fr.json`/`ar.json` for the button page's title/button/instructions text, RTL-aware. `tsc`/`next build` clean; local checks confirmed the GET page renders (200 HTML, no `Set-Cookie` — i.e. no session established) for both `type`s and both locales, `POST` with a fake token redirects to the correct type-aware error page, `/fr`/`/ar` still 200, and `scripts/verify-lifecycle.ts` still 42/42.
* **Production bug: signing up with an already-used email leaked a raw Postgres error (2026-07-15)**: `duplicate key value violates unique constraint "users_email_key"` was shown directly to the user. Root cause matches the report exactly: `supabase.auth.signUp()` deliberately succeeds (no error) for an email that's already registered — Supabase's own documented enumeration-safety behavior, confirmed via `search_docs` ("If you try to create an email account after previously signing up... you'll receive an obfuscated user response... prevents user enumeration attacks") — but the follow-up service-role insert into `public.users` was not idempotent, so it 23505'd on either a repeat unconfirmed signup (same `id`, `users_pkey`) or a genuinely-already-registered email (`signUp()`'s obfuscated-user `id` differs from the real account's, so the conflict lands on `users_email_key` instead). **Fix**: both `23505` shapes are now a success path — redirect to the same generic `signup=success` outcome as a real new signup, no error shown, no schema change. Classification for logging only (`auth_signup_existing_email` via `logAuthEvent`, `outcome: "success"`) is derived from whether the Postgrest error's `details` mentions `email`, without ever logging the raw `message`/`details` text itself (both embed the literal email value — logged only `actorId` (the `auth.users` id) and a hand-written classification string). Also closed a second, narrower instance of the same "raw backend error to the UI" class of bug found while auditing this function per the task: the pre-existing `signUp()`-error and any *other* profile-insert-error path both put the raw error message straight into the redirect URL (`/signup?error=${encodeURIComponent(error.message)}`), which the signup page's already-existing fallback rendering (for any error code not in its known-codes list) shows verbatim — both paths now redirect to the existing generic `signupFailed` code instead, with the real message going only to `logAuthEvent`. `tsc`/`next build` clean; local checks: fresh-email signup succeeds, immediately repeating the exact same signup shows the identical generic success message with no error, `scripts/verify-lifecycle.ts` still 42/42 (unaffected — it creates test users via the service-role admin API directly, not through this action).
* **Production bug: confirm button worked (verifyOtp succeeded) but the browser showed a 405 (2026-07-15)**: root cause was exactly what it looked like — every redirect issued from `POST /auth/confirm` (both success paths and the shared `errorRedirect` helper) used `NextResponse.redirect()` with no explicit status, which defaults to `307` (confirmed by reading Next.js's own installed source, `node_modules/next/dist/server/web/spec-extension/response.js`: `const status = ... ?? 307`). `307` preserves the request method, so the browser re-issued the redirect as a **POST** against the destination page (`/reset-password`, `/signup`, `/login`, `/dashboard`, the locale homepage) — none of which have a `POST` handler → `405`. This is the standard PRG (Post/Redirect/Get) pitfall: a POST handler's redirect must be `303` (or `302`), never left at the `307`/`308` default, or the browser replays the original method against a destination that doesn't expect it. **Fix**: `errorRedirect()` now takes an explicit `status: 307 | 303` parameter instead of leaving `NextResponse.redirect` to its default — `POST`'s three redirect call sites (the two `errorRedirect` calls and the two success redirects) all pass `303`; `GET`'s two `errorRedirect` calls pass `307` explicitly (harmless there — the follow-up request is a GET either way — but now a deliberate choice, not an implicit default, so this class of bug can't silently reappear if the shared helper's default ever changes). `tsc`/`next build` clean; local `curl -i` checks confirmed every `POST` redirect (missing-params, failed-verify for both `type`s, and — implicitly, since the code path is identical — the success paths) returns `303`, `curl -L` following the full chain lands on `200` with no `405` anywhere, `GET`'s redirects are `307`, `/fr`/`/ar` still `200`, `scripts/verify-lifecycle.ts` still 42/42.
...
- Signup (`lib/auth/actions.ts`) picks a role (`creator` or `brand` —
  ...
  the `public.users` row right after `supabase.auth.signUp()`. **That
  insert must tolerate `23505` (unique violation) as a success path, not an
  error** (fixed 2026-07-15 — see History): `signUp()` intentionally
  succeeds — no error — when called with an email that's already
  registered (Supabase's own enumeration-safety behavior: an "obfuscated
  user" response, no new row, no new email sent for an existing confirmed
  account), so the profile insert can collide two different, both-benign
  ways — same `id` (a repeat signup attempt on a still-unconfirmed
  account, `users_pkey`) or a different `id` with the same `email`
  (`users_email_key` — this was the exact production bug:
  `duplicate key value violates unique constraint "users_email_key"` shown
  raw to the user). Either shape redirects to the *same* generic
  `signup=success` outcome as a genuine new signup — showing anything
  different (an error banner, a distinct message) would let an attacker
  enumerate registered emails by reading the response, exactly the
  attack `signUp()`'s own behavior is designed to prevent. Classifying
  which of the two shapes occurred is for the log only
  (`auth_signup_existing_email` via `logAuthEvent`, keyed off whether
  the Postgrest error's `details` mentions `email` — never the raw
  `message`/`details` themselves, both of which embed the actual email
  value). Any *other* `signUp()` or profile-insert failure now redirects
  to the same generic `signupFailed` code — the real error only ever goes
  to `logAuthEvent`, never into the URL/UI (the raw-message-in-the-URL
  pattern this replaces was a second, narrower instance of the same class
  of bug as the `users_email_key` leak, not a separate one).
- **`app/auth/confirm/route.ts`** handles both email-confirmation and
  password-recovery links (fixed 2026-07-15, hardened again 2026-07-15 —
  see History). It deliberately lives outside `app/[locale]/` — Supabase's
  email templates embed one fixed link with no locale variable, so
  `proxy.ts`'s matcher explicitly excludes `/auth`. It uses the
  `token_hash` + `supabase.auth.verifyOtp()` pattern (verified against
  current Supabase docs via the Supabase MCP connector's `search_docs`, not
  pattern-matched from memory) — **not** `exchangeCodeForSession(code)`,
  which this route used to call and which is the wrong tool here: that PKCE
  flow needs a `code_verifier` cookie set on the *same browser* that
  originally requested the email, which is never true for password reset in
  the normal case (request on desktop, open the email on a phone, or vice
  versa) — it silently failed with `auth_callback_failed` every time the
  link was opened somewhere else.
- **GET vs POST split — GET must never call `verifyOtp`.** Email providers'
  security scanners (Gmail, Outlook Safe Links, etc.) prefetch links in
  incoming mail with a plain GET before the real user opens the email; if
  that GET redeemed the one-time token, the scanner consumes it and the
  real click fails. `GET /auth/confirm` only validates `token_hash`/`type`
  are present and renders a minimal, dependency-light HTML page (built by
  hand in the route handler, not a React page, since the URL must stay
  locale-less but the button text is locale-aware via `next`) with a button
  that `POST`s the same params back to `/auth/confirm`. Only the `POST`
  handler calls `verifyOtp` — a scanner's GET never triggers it. This was a
  real, confirmed production bug: the previous GET-only version logged
  users in immediately on prefetch and, for recovery specifically, could
  land signed-in on the dashboard instead of ever showing a password form.
  Required Supabase dashboard edits, by hand (templates aren't in
  ...
  On a successful `POST` verify, **`type=recovery` always redirects to
  `/${locale}/reset-password`, ignoring `next` entirely** — recovery must
  land on the password form, never anywhere else (not `next`, not a
  dashboard-ish default), since changing the password is the one thing the
  flow exists for. `type=email`/`signup` keeps the sanitized `next`,
  falling back to the bare locale homepage (not the dashboard) if `next` is
  missing. On failure, the destination is picked by `type`: a failed
  ...
  (sign up again), anything else (including a legacy `?code=`-style link
  with no `token_hash`) falls back to `/login?error=auth_confirm_failed`.
  Every outcome is logged via `logAuthEvent()` (`lib/log.ts`, same
  `AUTH_EVENT`-tag convention as `logMoneyEvent`) — `confirmRender` (GET)
  and `confirmRedeem` (POST) are logged as distinct actions specifically so
  a scanner's prefetch (a `confirmRender` with no matching `confirmRedeem`)
  is visible in Vercel's log viewer as different from a real completed
  flow; neither ever logs the token itself, only whether it was present.
